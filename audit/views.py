...
def audit_log_list(request):
    ...
    if otp_status == "used":
        logs = logs.filter(
            otp_usage__isnull=False,
        )

    elif otp_status == "not_used":
        logs = logs.filter(
            otp_usage__isnull=True,
        )

    elif otp_status == "valid":
        logs = logs.filter(
            otp_usage__was_valid=True,
        )

    elif otp_status == "invalid":
        logs = logs.filter(
            otp_usage__was_valid=False,
        )
    ...
    if search:
        logs = logs.filter(
            ...
            | Q(ticket__customer_name_snapshot__icontains=search)
            | Q(ticket__customer_phone_snapshot__icontains=search)
            | Q(ticket__service_name_snapshot__icontains=search)
            | Q(ticket__prepaid_plan_name_snapshot__icontains=search)
            | Q(ticket__prepaid_description__icontains=search)
            | Q(ticket__parking_mode__icontains=search)
